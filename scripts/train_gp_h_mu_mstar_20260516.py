"""
Train the current thesis GP surrogate after AL batch2.
...
ANALYSIS_ID = "gp_h_mu_mstar_20260516"
OUT_DIR = PROJECT_ROOT / "data" / "analysis" / ANALYSIS_ID
FIG_DIR = OUT_DIR / "figures"
MODEL_DIR = PROJECT_ROOT / "models" / "surrogates"
MODEL_PATH = MODEL_DIR / "gp_h_mu_mstar_after_al2_20260516.pkl"
...
def recommended_al3(pipe) -> pd.DataFrame:
    rows = [
        ("al3_lowH_m085_mu0572", 0.175, 0.572, 0.85, "mid bracket H=0.175,m*=0.85 between 0.560 F and 0.585 E"),
        ("al3_base_m085_mu0800", 0.200, 0.800, 0.85, "mid bracket H=0.200,m*=0.85 between 0.780 F and 0.820 E"),
        ("al3_midH_m085_mu0900", 0.210, 0.900, 0.85, "try upper closure; 0.880 is barely F"),
        ("al3_midH_m100_mu0755", 0.210, 0.755, 1.00, "mid bracket H=0.210,m*=1.00 between 0.740 F and 0.770 E"),
        ("al3_highH_m100_mu0870", 0.225, 0.870, 1.00, "mid bracket H=0.225,m*=1.00 between 0.860 F and 0.880 E"),
        ("al3_highH_m115_mu0760", 0.225, 0.760, 1.15, "lower/mid bracket H=0.225,m*=1.15 between 0.740 F and 0.800 E"),
        ("al3_highH_m115_mu0780", 0.225, 0.780, 1.15, "upper/mid bracket H=0.225,m*=1.15 before 0.800 E"),
        ("al3_highH_m125_mu0700", 0.225, 0.700, 1.25, "mid bracket H=0.225,m*=1.25 between 0.660 F and 0.740 E"),
    ]
    df = pd.DataFrame(
        rows,
        columns=[
...
def save_figures(used: pd.DataFrame, loo: pd.DataFrame, grid: pd.DataFrame, al3: pd.DataFrame) -> None:
    plt.rcParams.update(
        {
            "font.size": 10,
            "axes.grid": True,
    ...
    ax.set_title("Validacion leave-one-out del GP after-AL2")
    ...
    for ax, m in zip(axes.ravel(), mass_slices):
        ...
        cand = al3[np.isclose(al3["boulder_mass"], m)]
    ...
    fig.suptitle("Frontera GP after-AL2 y candidatos AL3", y=0.98)
    ...
    fig.suptitle("Incertidumbre del GP after-AL2", y=0.98)
...
def write_readme(summary: DatasetSummary, metrics: dict, al3: pd.DataFrame) -> None:
    text = f"""# GP H-mu-mstar after AL2
...
def main() -> None:
    ...
    al3.to_csv(OUT_DIR / "al3_candidates.csv", index=False)
    al3[
        [
            "case_id",
            "dam_height",
    ...
    ].to_csv(OUT_DIR / "al3_matrix_recommended.csv", index=False)
    ...
    save_figures(used, loo, grid, al3)
    ...
    write_readme(summary, metrics, al3)
