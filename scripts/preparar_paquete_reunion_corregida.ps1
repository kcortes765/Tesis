...
if (-not $OutputDir) {
    $OutputDir = Join-Path $ProjectRoot "docs\reunion_profe_$Today"
}
...
$filesToCopy = @(
    ...
    @{ Src = Join-Path $ProjectRoot "data\figures\conv2_003_vs_002.png"; Dst = "11_conv2_003_vs_002.png" },
    @{ Src = Join-Path $ProjectRoot "data\figures\conv2_forces_detail.png"; Dst = "12_conv2_forces_detail.png" },
    @{ Src = Join-Path $ProjectRoot "data\figures\conv2_solver_detail.png"; Dst = "13_conv2_solver_detail.png" },
    @{ Src = Join-Path $ProjectRoot "data\figures\conv2_vs_conv3.png"; Dst = "14_conv2_vs_conv3.png" },
    @{ Src = Join-Path $ProjectRoot "data\figures\conv3_dashboard.png"; Dst = "15_conv3_dashboard.png" }
)
...
foreach ($item in $filesToCopy) {
    if (Test-Path $item.Src) {
        Copy-Item $item.Src (Join-Path $OutputDir $item.Dst) -Force
        $copied.Add($item.Dst) | Out-Null
    }
    else {
        $missing.Add($item.Dst) | Out-Null
    }
}

$readme = @'
# Paquete de reunion con profesor

Este paquete reune lo minimo necesario para mostrar el estado actual del cierre de dp
con la geometria corregida del bloque.

## Orden sugerido de presentacion

1. 01_corrected_scout_overview.png
   Muestra el scouting corregido completo y la banda critica en mu.
...
5. 00_registro_objetivo.md
   Contiene el registro tecnico objetivo de lo que ocurrio, las decisiones tomadas y el estado actual.
...
$missingList = if ($missing.Count -gt 0) { ($missing | ForEach-Object { "- $_" }) -join "`r`n" } else { "- Sin faltantes." }
...
## Archivos no encontrados al momento de armar el paquete
$missingList
"@

Set-Content -Path (Join-Path $OutputDir "00_LEEME.md") -Value $readme -Encoding UTF8
Set-Content -Path (Join-Path $OutputDir "00_registro_objetivo.md") -Value $registro -Encoding UTF8
Set-Content -Path (Join-Path $OutputDir "99_inventario.md") -Value $inventario -Encoding UTF8
...
Write-Host ""
Write-Host "Paquete de reunion creado:" -ForegroundColor Green
