"""
Train the current thesis GP surrogate after AL batch4.
...
ANALYSIS_ID = "gp_h_mu_mstar_after_al4_20260520"
OUT_DIR = PROJECT_ROOT / "data" / "analysis" / ANALYSIS_ID
FIG_DIR = OUT_DIR / "figures"
MODEL_DIR = PROJECT_ROOT / "models" / "surrogates"
MODEL_PATH = MODEL_DIR / "gp_h_mu_mstar_after_al4_20260520.pkl"
...
EXPORTS = [
    ...
    ("al4", "exports/al_batch4_after_al3_20260520/al_batch4_summary.csv"),
]
...
def recommended_al5(pipe) -> pd.DataFrame:
    rows = [
        ("al5_highH_m085_mu0900", 0.225, 0.900, 0.85, "test whether high-H low-mass frontier is inside the current mu domain"),
        ("al5_base_m085_mu0795", 0.200, 0.795, 0.85, "close base low-mass bracket between 0.790 F and 0.800 E"),
        ("al5_midH_m100_mu0744", 0.210, 0.744, 1.00, "close mid-H reference-mass bracket between 0.740 F and 0.748 E"),
        ("al5_highH_m100_mu08625", 0.225, 0.8625, 1.00, "close high-H reference-mass bracket between 0.860 F and 0.865 E"),
        ("al5_midH_m115_mu0700", 0.210, 0.700, 1.15, "find lower side of missing mid-H m*=1.15 bracket"),
        ("al5_highH_m115_mu0755", 0.225, 0.755, 1.15, "close high-H m*=1.15 bracket between 0.750 F and 0.760 E"),
        ("al5_midH_m125_mu0660", 0.210, 0.660, 1.25, "fill missing mid-H m*=1.25 cut and test expected transition region"),
        ("al5_highH_m125_mu0690", 0.225, 0.690, 1.25, "close high-H m*=1.25 bracket between 0.680 F and 0.700 E"),
    ]
    df = pd.DataFrame(
        rows,
        columns=[
...
def save_figures(used: pd.DataFrame, loo: pd.DataFrame, grid: pd.DataFrame, al5: pd.DataFrame) -> None:
    plt.rcParams.update(
        {
            "font.size": 10,
            "axes.grid": True,
    ...
    ax.set_title("Validacion leave-one-out del GP after-AL4")
    ...
    for ax, m in zip(axes.ravel(), mass_slices):
        ...
        cand = al5[np.isclose(al5["boulder_mass"], m)]
    ...
    fig.suptitle("Frontera GP after-AL4 y candidatos AL5", y=0.98)
    ...
    fig.suptitle("Incertidumbre del GP after-AL4", y=0.98)
...
def write_readme(summary: DatasetSummary, metrics: dict, al5: pd.DataFrame) -> None:
    text = f"""# GP H-mu-mstar after AL4
...
def main() -> None:
    ...
    al5.to_csv(OUT_DIR / "al5_candidates.csv", index=False)
    al5[
        [
            "case_id",
            "dam_height",
    ...
    ].to_csv(OUT_DIR / "al5_matrix_recommended.csv", index=False)
    ...
    save_figures(used, loo, grid, al5)
    ...
    write_readme(summary, metrics, al5)
